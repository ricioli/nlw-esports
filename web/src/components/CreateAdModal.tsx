...
const CreatedAdModal = () => {
  const [games, setGames] = useState<Game[]>([]);
  const [weekDays, setWeekDays] = useState<string[]>([]);
  const [useVoiceChannel, setUseVoiceChannel] = useState<boolean>(false);

  useEffect(() => {
    axios('http://localhost:3333/games').then(({ data }) => setGames(data.data));
  }, []);
  ...
  return (
    <Dialog.Portal>
      ...
      <Dialog.Content className="fixed bg-[#2a2634] py-8 px-10 text-white top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2 rounded-large shadow-black/25">
        ...
        <form onSubmit={handleCreateAd} className="mt-8 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            ...
            <Input name="name" id="name" placeholder="Como te chamam destro do game?" />
          </div>
